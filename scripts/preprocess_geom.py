...
def preprocess_geom_drugs(
    ...
    for split_name in ["train", "val", "test"]:
        ...
        print(
            f"  Loaded {initial_molecules:,} molecules, {initial_conformers:,} conformers"
        )

        # Prepare args for parallel processing
        args_list = [
            (smiles, mols, skip_topology, topology_tolerance) for smiles, mols in data
        ]
        ...
        for result in results:
            ...
            for conf_data in result:
                ...
                all_atom_types.append(
                    atomic_numbers_to_symbols(conf_data["atomic_numbers"])
                )
        ...
        print(
            f"  Kept {final_molecules:,} molecules ({final_molecules / initial_molecules * 100:.1f}%)"
        )
        print(
            f"  Kept {final_conformers:,} conformers ({final_conformers / initial_conformers * 100:.1f}%)"
        )
