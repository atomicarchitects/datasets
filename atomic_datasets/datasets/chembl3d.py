...
class ChEMBL3D(datatypes.MolecularDataset):
    ...
    def preprocess(self):
        ...
        self._positions = np.load(
            self._get_file_path(f"{prefix}_positions.npy"), mmap_mode=self.mmap_mode
        )
        self._species = np.load(
            self._get_file_path(f"{prefix}_species.npy"), mmap_mode=self.mmap_mode
        )
        self._offsets = np.load(
            self._get_file_path(f"{prefix}_offsets.npy")
        )  # Small, load fully into RAM
        self._charges = np.load(
            self._get_file_path(f"{prefix}_charges.npy"), mmap_mode=self.mmap_mode
        )
        self._is_aromatic = np.load(
            self._get_file_path(f"{prefix}_is_aromatic.npy"), mmap_mode=self.mmap_mode
        )
        self._is_in_ring = np.load(
            self._get_file_path(f"{prefix}_is_in_ring.npy"), mmap_mode=self.mmap_mode
        )
        self._hybridization = np.load(
            self._get_file_path(f"{prefix}_hybridization.npy"), mmap_mode=self.mmap_mode
        )
    ...
    def __getitem__(self, idx: int) -> datatypes.Graph:
        ...
        if idx < 0 or idx >= len(self._indices):
            raise IndexError(
                f"Index {idx} out of range for dataset of size {len(self._indices)}"
            )
