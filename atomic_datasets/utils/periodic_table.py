...
class PeriodicTable:
    ...
    def __init__(self):
        ...
        self.blocks = np.array(
            ...
            + [0] * 2
            + [1] * 6  # Period 2: Li-Be (s), B-Ne (p)
            + [0] * 2
            + [1] * 6  # Period 3: Na-Mg (s), Al-Ar (p)
            + [0] * 2
            + [2] * 10
            + [1] * 6  # Period 4: K-Ca (s), Sc-Zn (d), Ga-Kr (p)
            + [0] * 2
            + [2] * 10
            + [1] * 6  # Period 5: Rb-Sr (s), Y-Cd (d), In-Xe (p)
            + [0] * 2
            + [3] * 14
            + [2] * 10
            + [1] * 6  # Period 6: Cs-Ba (s), La-Yb (f), Hf-Hg (d), Tl-Rn (p)
            + [0] * 2
            + [3] * 14
            + [2] * 10
            + [1] * 6  # Period 7: Fr-Ra (s), Ac-No (f), Rf-Cn (d), Nh-Og (p)
        )
        ...
        self.groups = np.array(
            ...
            + [0, 1]
            + [2] * 15
            + list(range(3, 18))  # Period 6 (La-Lu in group 2 placeholder)
            + [0, 1]
            + [2] * 15
            + list(range(3, 18))  # Period 7 (Ac-Lr in group 2 placeholder)
        )
