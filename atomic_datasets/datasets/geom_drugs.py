...
GEOM_DRUGS_ZENODO_URL = (
    "https://zenodo.org/record/18484634/files/geom_drugs_processed.zip"
)
...
class GEOMDrugs(datatypes.MolecularDataset):
    ...
    def __init__(
        ...
        if conformer_selection not in ("first", "random", "all"):
            raise ValueError(
                f"conformer_selection must be 'first', 'random', or 'all', got '{conformer_selection}'"
            )
    ...
    def _ensure_downloaded(self):
        ...
        if not os.path.exists(zip_path):
            ...
            utils.download_url(
                GEOM_DRUGS_ZENODO_URL, self.root_dir, filename=zip_filename
            )
    ...
    def _load_data(self):
        ...
        self._mol_indices = np.load(
            os.path.join(self.root_dir, f"{prefix}_mol_indices.npy")
        )
